...
def scan_subdomain(domain):
    # Menampilkan pesan bahwa pemindaian subdomain sedang berlangsung
    print(f"Memindai subdomain untuk domain '{domain}'...")

    try:
        # Mendapatkan daftar alamat IP untuk subdomain
        answers = dns.resolver.resolve(domain, 'A')
        if answers:
            # Jika ada hasil, menampilkan subdomain yang ditemukan
            print("Subdomain yang ditemukan:")
            for rdata in answers:
                print(f"- {domain}: {rdata}")
        else:
            # Jika tidak ada hasil, menampilkan pesan bahwa tidak ada subdomain yang ditemukan
            print("Tidak ada subdomain yang ditemukan.")
    except dns.resolver.NoAnswer:
        # Menangani jika tidak ada jawaban dari DNS untuk subdomain
        print("Tidak ada subdomain yang ditemukan.")
    except dns.resolver.NXDOMAIN:
        # Menangani jika domain tidak ditemukan
        print("Domain tidak ditemukan.")
    except dns.resolver.Timeout:
        # Menangani jika waktu habis saat meminta DNS
        print("Waktu habis saat meminta DNS.")
